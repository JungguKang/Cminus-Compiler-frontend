  ...
  while (tree != NULL) {
    ...
    { switch (tree->kind.stmt) {
		...
		case VarK:
			...
			{
				fprintf(listing,"Array declaration, name : %s, size : %d, type : ",tree->attr.name,tree->size);
				if(tree->type == 0)
					fprintf(listing,"void \n");
				else
					fprintf(listing,"int \n");
    ...
    { switch (tree->kind.exp) {
        ...
        case IdK:
		  ...
		  if(tree->size != 1)
			  fprintf(listing, "array size : %d",tree->size);
		  fprintf(listing,"\n");
          break;
		case IdK2:
		  fprintf(listing,"Array Id: %s of index\n", tree->attr.name);
		  break;
        default:
          fprintf(listing,"Unknown ExpNode kind\n");
          break;
